...
from a_utils import compute_metrics, plot_preds, forward_and_jac, EITDataModule, TVLoss, get_neighbor,read_hypers,compute_aplpha
#%
class LearnedPDEIT(pl.LightningModule):
    def __init__(self, shape_primal,shape_dual,grad_type,hypers_eit,hypers_model):
        super().__init__()
        self.mse_loss = nn.MSELoss()
        self.grad_type = grad_type
        self.hypers_eit = hypers_eit
        self.hypers_model = hypers_model
        # 测试相同参数的情况: 就测试lpd的baseline和lstm模型（lstm模型的参数是baseline的参数的两倍左右）
        test_equal_params = False
        if test_equal_params:
            n_iter = 22 if grad_type=='baseline' else 10
            self.model = LearnedPrimalDual(forward_and_jac, shape_primal,shape_dual,
                                           grad_type,hypers_eit,hypers_model,n_iter=n_iter)
        else:
            self.model = LearnedPrimalDual(forward_and_jac, shape_primal,shape_dual,
                                           grad_type,hypers_eit,hypers_model)
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.Adam(self.parameters(),
                                     lr=0.001,
                                     betas=(0.9,0.99))
        reduce_on_plateu = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=10001) #xxx
        schedulers = {
...
if __name__ == '__main__':
    ...
    parser.add_argument('--h0', type=float, default=0.07, help='initial mesh size')  # xxx 场景参数

    parser.add_argument('--n_train', type=int, default=50, help='the number of training data')  # xxx 实验参数
    parser.add_argument('--n_epoch', type=int, default=10, help='the number of epoches')  # xxx 实验参数
    ...
    parser.add_argument('--reg', type=float, default=0.0, help='reg of tv loss')  # xxx 寻优参数
    parser.add_argument('--anderson_m', type=int, default=3, help='')  # xxx 实验参数
    parser.add_argument('--idx_grad', type=int, default=2, help='index of grad_type')  # xxx 寻优参数
    args = parser.parse_args()
    pl.seed_everything(args.n_test)
    print(args)

    num_inclusion = args.n_inclusion               # xxx
    h0 = args.h0
    ...
    hypers_model['anderson_m'] = args.anderson_m
    ...
    dataset = EITDataModule(train_data_file_list=train_data_file_list,
                            ...
                            batch_size=1)
    #                0            1        2        3          4      5           6            7
    grad_type = ['baseline', 'momentum', 'lstm', 'lstm_low', 'gru','gru_low','anderson','anderson_dual'][args.idx_grad]
    method = f'gt_{grad_type}_l_{args.layer}_reg_{args.reg}_test_{args.n_test}'      #xxx
    if args.n_inclusion==2:
        log_dir = os.path.join(*[f'exp-sameParaNum-lpd', f"{args.n_train}_"+data_type, method])
    if args.n_inclusion==4:
        log_dir = os.path.join(*[f'exp-lpd-sameParaNum', f"{args.n_train}_"+data_type,method])
    # ----------load trained model and re-run------------------
    chkp_path = None
    # version = 'version_10'    #xxx
    # chkp_path_reg = os.path.join(*[log_dir,'*',version,'checkpoints','last.ckpt'])
    # try:
    ...
    model = LearnedPDEIT(shape_primal,shape_dual,grad_type,hypers_eit,hypers_model)
    print(model)
    print('-'*60)
