...
class LearnedPD(pl.LightningModule):
    def __init__(self, shape_primal, shape_dual, grad_type, hypers):
        ...
        self.hypers = hypers
        self.model = LearnedPrimalDual(forward_and_jac, shape_primal, shape_dual, hypers, grad_type)
    ...
    def training_step(self, batch, batch_idx):
        ...
        ckpt_step_path = os.path.join(self.trainer.default_root_dir,
                                      'lightning_logs','version_0','checkpoints',
                                      f'{self.trainer.global_step}'.zfill(3)+'.ckpt')
        if self.global_step>30 and self.global_step<150:
            if self.global_step%1==0:
                self.trainer.save_checkpoint(ckpt_step_path)
        else:
